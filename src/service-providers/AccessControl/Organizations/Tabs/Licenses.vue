...
import DialogAssignLicense from "../Dialogs/DialogAssignLicense.vue";
...
const isDialogAssignLicenseOpen: Ref<boolean> = ref(false);
...
const handleAssignLicenseDialog = () => {
  isDialogAssignLicenseOpen.value = true;
};
...
<template>
  <div>
    <m-m-expandable-card title="Licenses" cy="licenses-expandable">
      <m-m-table
        show-search
  ...
  />
  <DialogAssignLicense
    v-if="licenses && isDialogAssignLicenseOpen"
    :key="key"
    :is-open="isDialogAssignLicenseOpen"
    :customer="props.user"
    :service-provider-id="serviceProviderId"
    :licenses="licenses.results"
    @close-dialog="isDialogAssignLicenseOpen = false"
