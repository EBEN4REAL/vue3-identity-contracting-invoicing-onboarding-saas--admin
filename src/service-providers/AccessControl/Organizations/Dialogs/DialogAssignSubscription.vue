...
const submit = async () => {
  try {
    ...
    alertText.value = "Error assigning subscriptions";
  } finally {
...
<template>
  <m-m-dialog
    :is-open="isOpen"
    :title="`Allocate subscriptions to ${customer.organization?.name}`"
    icon="document-check-add"
    cy="allocate-subscription-dialog"
    :has-divider="true"
    @close="onDialogClose"
  >
    <template #default>
      <m-m-alert
        v-if="isAlertVisible"
        status="error"
        cy="allocate-subscription-alert"
        class="mm-mb-12"
      ...
      <form v-mm-focus-first class="mm-page-dialog-form mm-mt-10">
        <div>
          <m-m-autocomplete
            ...
            label="Search & allocate subscriptions"
            cy="allocate-subscription-autocomplete"
            ...
            <template #option="{ item }">
              <div :class="selectLicenseOptionClassList(item.disabled)">
                ...
                <div
                  ...
                  :data-cy="`subscription-allocation-disabled-item-${item.value}`"
                >
                  <m-m-badge
                    class="mm-ml-3"
                ...
                <m-m-tooltip v-if="item.hasNoDefaultPolicies">
                  Can't allocate this subscription because there are no default
                  policies attached. Attach a default policy first and then you
                  can assign it.
                </m-m-tooltip>
              </div>
            </template>
