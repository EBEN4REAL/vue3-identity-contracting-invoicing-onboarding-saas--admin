...
const isLicenseEditable = (license: OauthClientRead): boolean =>
  license?.editable ?? true;

const isLicenseDeletable = (license: OauthClientRead): boolean =>
  license?.deletable ?? true;

const dropdownItems = (row: OauthClientRead): TypeDropdownItem[] => [
  {
    label: "View",
    type: "link",
    to: `/sp/${serviceProviderId.value}/applications/${row.id}`,
  },
  {
    label: "Edit",
    type: "button",
  ...
  },
  {
    ...
    onClick: () => {
      handleApplicationForPolicyAssignment(row.id);
...
const handleDialogConfirmDeleteClose = () => {
...
const goToNewApplication = () =>
  router.push({ path: `/sp/${serviceProviderId.value}/applications/new` });

const buttonItems: ComputedRef<Button[]> = computed(() => {
  const items = [];
...
const buttons = computed(() => [
  {
    key: "add_button",
    isDropdown: true,
    dropdownItems: isCWApplicationRoleBased.value ? buttonItems.value : [],
...
<template>
    ...
    >
      ...
      <template #url="{ row }">
        <m-m-link :href="row.url ? (row.url as string) : ''">
          <m-m-text-ellipsis max-width="200px">{{ row.url }}</m-m-text-ellipsis>
        </m-m-link>
      </template>
      <template #type="{ row }">
  ...
  <dialog-confirm-delete-application
    ...
    @close="handleDialogConfirmDeleteClose"
